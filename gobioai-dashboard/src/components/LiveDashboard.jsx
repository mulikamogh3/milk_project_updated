...
export default function LiveDashboard() {
  ...
  // NEW: State to track the exact lifecycle of our commands
  const [activeCommand, setActiveCommand] = useState({ id: null, command: null, status: 'IDLE' });
  const wsRef = useRef(null);

  // 1. Fetch Live Data (Existing)
  useEffect(() => {
  ...
  // 2. Fetch AI Predictions (Existing)
  useEffect(() => {
  ...
  // 3. NEW: WebSocket Listener for Real-Time Command Acknowledgements
  useEffect(() => {
    const ws = new WebSocket(`${WS_BASE}/ws/dashboard`);
    
    ws.onmessage = (event) => {
      const data = JSON.parse(event.data);
      // If the ESP32 replies to our specific command, update the UI instantly!
      if (data.type === 'COMMAND_RESULT' && data.command_id === activeCommand.id) {
        setActiveCommand(prev => ({ ...prev, status: data.status }));
        
        // Reset the UI after 3 seconds if it was successful
        if (data.status === 'EXECUTED' || data.status === 'REJECTED') {
          setTimeout(() => setActiveCommand({ id: null, command: null, status: 'IDLE' }), 3000);
        }
      }
    };
  ...
  // 4. NEW: Send Command Function
  const triggerCommand = async (commandName) => {
    try {
      // Step 1: Immediately show PENDING to the user
      setActiveCommand({ id: null, command: commandName, status: 'PENDING' });
      
      // Step 2: Hit the REST API
      const response = await axios.post(`${API_BASE}/device/${MACHINE_ID}/commands`, {
        command: commandName,
        parameters: {}
      });
      
      // Step 3: Upgrade to SENT status using the new tracking UUID
      setActiveCommand({ 
      ...
      setActiveCommand({ id: null, command: commandName, status: 'FAILED' });
      setTimeout(() => setActiveCommand({ id: null, command: null, status: 'IDLE' }), 3000);
    }
  };
    ...
    <div className="flex flex-col gap-6 max-w-6xl mx-auto animate-in fade-in duration-500 pb-10">
      ...
      <div className="bg-slate-800 p-6 rounded-3xl border border-slate-700/50 shadow-xl">
        ...
        <div className="flex flex-wrap gap-4 items-center">
            {/* Action Buttons */}
            <button 
              onClick={() => triggerCommand('AUTO_START')}
              disabled={activeCommand.status !== 'IDLE'}
              className="bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 text-white px-6 py-3 rounded-xl font-bold transition-colors flex items-center gap-2"
            >
              ▶ START BATCH
            </button>
            <button 
              onClick={() => triggerCommand('AUTO_STOP')}
              disabled={activeCommand.status !== 'IDLE'}
              className="bg-slate-600 hover:bg-slate-500 disabled:bg-slate-700 text-white px-6 py-3 rounded-xl font-bold transition-colors flex items-center gap-2"
            >
              ⏸ STOP
            </button>
            <button 
              ...
              disabled={activeCommand.status !== 'IDLE'}
              className="bg-red-600 hover:bg-red-500 disabled:bg-red-900/50 text-white px-6 py-3 rounded-xl font-bold transition-colors flex items-center gap-2 ml-auto"
            >
            ...
            {activeCommand.status !== 'IDLE' && (
              <div className="w-full mt-4 p-4 rounded-xl bg-slate-900 border border-slate-700 flex items-center justify-between">
                <div className="flex items-center gap-3 text-slate-300">
                  <span className="font-mono text-blue-400 text-sm">{activeCommand.id || 'GENERATING_ID'}</span>
                  <span>Executing: <strong>{activeCommand.command}</strong></span>
                </div>
                
                <div className="flex items-center gap-2 font-bold">
                  {activeCommand.status === 'PENDING' && <><Loader2 className="w-5 h-5 animate-spin text-yellow-400"/> <span className="text-yellow-400">PENDING...</span></>}
                  {activeCommand.status === 'SENT' && <><Send className="w-5 h-5 animate-bounce text-blue-400"/> <span className="text-blue-400">SENT TO EDGE...</span></>}
                  {activeCommand.status === 'EXECUTED' && <><CheckCircle2 className="w-5 h-5 text-green-400"/> <span className="text-green-400">CONFIRMED BY HARDWARE</span></>}
                  {activeCommand.status === 'REJECTED' && <><XCircle className="w-5 h-5 text-red-400"/> <span className="text-red-400">REJECTED BY SAFETY LOGIC</span></>}
                  {activeCommand.status === 'FAILED' && <><XCircle className="w-5 h-5 text-red-400"/> <span className="text-red-400">NETWORK FAILURE</span></>}
                </div>
              </div>
            )}
        </div>
